...
void _triangulos3D::draw_texturizado(){
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	glLightModeli(GL_LIGHT_MODEL_COLOR_CONTROL,GL_SEPARATE_SPECULAR_COLOR);
	glEnable(GL_NORMALIZE);
	glBegin(GL_TRIANGLES);
	for(int i=0;i<caras.size();++i){
		glTexCoord2f(texturas[caras[i]._0].x,texturas[caras[i]._0].y);
		glVertex3fv((GLfloat *) &vertices[caras[i]._0]);
		glTexCoord2f(texturas[caras[i]._1].x,texturas[caras[i]._1].y);
		glVertex3fv((GLfloat *) &vertices[caras[i]._1]);
		glTexCoord2f(texturas[caras[i]._2].x,texturas[caras[i]._2].y);
		glVertex3fv((GLfloat *) &vertices[caras[i]._2]);
	}
	glEnd();

}
void _triangulos3D::draw_texturizado_suave(){
	glShadeModel(GL_SMOOTH);
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	glLightModeli(GL_LIGHT_MODEL_COLOR_CONTROL,GL_SEPARATE_SPECULAR_COLOR);
	glEnable(GL_NORMALIZE);
...
void _triangulos3D::draw_texturizado_flat(){
...
//**************************************************************************
// CALCULO DE NORMALES PLANO Y VERTICES
//
//***************************************************************************
...
void _triangulos3D::calculo_normales_vertices(){
	calculo_normales_plano();
	normales_vertices.resize(vertices.size());	
	for (int ixVertice = 0; ixVertice < vertices.size(); ixVertice++)
	{
		vector<int> caras_adyacentes;
		caras_adyacentes.resize(caras.size());
		int nCarasAdyacentes=0;
		//Compruebo los vertices adyacentes a vertice ixVertice actual
		for (int ix = 0; ix < caras.size(); ix++){
			if (caras[ix]._0 == ixVertice)
				caras_adyacentes[nCarasAdyacentes++] = ix;
			else
				if (caras[ix]._1 == ixVertice)
					caras_adyacentes[nCarasAdyacentes++] = ix;
				else
					if (caras[ix]._2 == ixVertice)
						caras_adyacentes[nCarasAdyacentes++] = ix;
		}
		
		//Por cada cara adyacente al vertice voy metiendo en normales_vertices la sumatoria normalizada a cada cara
		GLfloat pn[3];
		pn[0] = pn[1] = pn[2] = 0;
		for (int jx = 0; jx < nCarasAdyacentes; jx++)
		{
			int ixFace = caras_adyacentes[jx];
			pn[0] += normales[ixFace].x;
			pn[1] += normales[ixFace].y;
			pn[2] += normales[ixFace].z;
		}
		
		pn[0] /= nCarasAdyacentes;
		pn[1] /= nCarasAdyacentes;
		pn[2] /= nCarasAdyacentes;
		
		//Normalizo el vector
		GLfloat len = (GLfloat)(sqrt(pn[0]*pn[0] + pn[1]*pn[1] + pn[2]*pn[2]));
		normales_vertices[ixVertice].x = pn[0] / len;
		normales_vertices[ixVertice].y = pn[1] / len;
		normales_vertices[ixVertice].z = pn[2] / len;
	}

}
/////Distancia entre vertices
float _triangulos3D::distancia_total_vertices(){
	float dist=0;
	for(int i=1;i<vertices.size();++i){
		dist+=sqrt(((vertices[i].x-vertices[i-1].x)*(vertices[i].x-vertices[i-1].x))+ ((vertices[i].y-vertices[i-1].y)*(vertices[i].y-vertices[i-1].y)));
	}
	return dist;
}
...
void _triangulos3D::revolucionar_perfil(int secciones,bool tapa_sup,bool tapa_inf){
	...
	texturas.resize(tamfin);///LAS TEXTURAS
	finales.resize(tamfin);
	float alpha=0;
	
	
	float distancia_total=distancia_total_vertices();
	...
	for (int i = 0; i < tam; ++i){
		...
		for (int j = 0; j<secciones; j++){
			...
			texturas[(i*secciones)+j].y=finales[(i*secciones)+j].y/distancia_total;
			float alfa= atan(finales[(i*secciones)+j].x/finales[(i*secciones)+j].z);
			texturas[(i*secciones)+j].x=alfa/(2*M_PI);
			//cout<<"x "<<texturas[(i*secciones)+j].x<<" y"<<texturas[(i*secciones)+j].y<<endl;
		}
...
//**************************************************************************
// ELEMENTOS DIBUJABLES SUELO, CUBO, CIRCULO, PIRAMIDE, ETC
//
//
//***************************************************************************
...
_suelo::_suelo(float tam){
	vertices.resize(4);
	texturas.resize(4);
	texturas[3].x=0;texturas[3].y=0;
	texturas[2].x=1;texturas[2].y=0;
	texturas[1].x=0;texturas[1].y=1;
	texturas[0].x=1;texturas[0].y=1;
...
void _suelo::draw_suelo_texturizado(){
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	glLightModeli(GL_LIGHT_MODEL_COLOR_CONTROL,GL_SEPARATE_SPECULAR_COLOR);
	glEnable(GL_NORMALIZE);
	glBegin(GL_TRIANGLES);
...
_cubo::_cubo(float tam){
	...
	texturas[3].x=0;texturas[3].y=0;
	texturas[2].x=1;texturas[2].y=0;
	texturas[1].x=1;texturas[1].y=1;
	texturas[0].x=0;texturas[0].y=1;
...
void _cubo::draw_cubo_texturizado(){
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	glLightModeli(GL_LIGHT_MODEL_COLOR_CONTROL,GL_SEPARATE_SPECULAR_COLOR);
	glBegin(GL_TRIANGLES);
	int j[3];
	for(int i=0;i<caras.size();++i){
		...
		glTexCoord2f(texturas[j[0]].x,texturas[j[0]].y);
		glVertex3fv((GLfloat *) &vertices[caras[i]._0]);
		glTexCoord2f(texturas[j[1]].x,texturas[j[1]].y);
		glVertex3fv((GLfloat *) &vertices[caras[i]._1]);
		glTexCoord2f(texturas[j[2]].x,texturas[j[2]].y);
		glVertex3fv((GLfloat *) &vertices[caras[i]._2]);
